...
const initApp = async () => {
  ...
  const applyCreativeOutputRegexPairSafe = (source, options = {}) => {
    const protectedSource = protectUnclosedAutoImagePromptTags(source);
    const result = applyOutputRegexPairSafe(protectedSource.text, options) || {};
    return {
      stored: restoreProtectedAutoImagePromptTags(result.stored, protectedSource),
      display: restoreProtectedAutoImagePromptTags(result.display, protectedSource),
    };
  };
  const applyCreativeOutputDisplayRegexSafe = (source, options = {}) => {
    const protectedSource = protectUnclosedAutoImagePromptTags(source);
    const result = applyOutputDisplayRegexSafe(protectedSource.text, options);
    return restoreProtectedAutoImagePromptTags(result, protectedSource);
  };
    ...
    return list.map((m, i) => {
      ...
      if (m.role === 'assistant' && (m.type === 'text' || !m.type)) {
        if (m?.meta?.renderRich) {
          if (creativeSource) {
            const { stored, display } = applyCreativeOutputRegexPairSafe(creativeSource, {
              appBridge: window.appBridge,
              depth,
          ...
          return setCachedDecoratedMessage(m, sid, decorationSignature, {
            ...m,
            avatar,
            content: applyCreativeOutputDisplayRegexSafe(creativeBase, {
              appBridge: window.appBridge,
              depth,
              normalizeText: normalizeCreativeLineBreaksForDisplay,
            }),
...
`;
  ...
  const wallpaperOpacityInput = document.getElementById('wallpaper-opacity');
  ...
  const isAutoImagePromptOnlyMessage = (message = {}) => {
    ...
    if (!sessionId) {
      ...
      return false;
	    ...
	    const sourceMessageId = sourceMessageIdOverride === undefined
	      ? sender.sourceMessageId
	      : String(sourceMessageIdOverride || '').trim();
	    const replacementMessageId = String(replaceMessageId || '').trim();
	    const replacementMessage = replacementMessageId
	      ? chatStore.findMessage(replacementMessageId, sessionId)
	      : null;
	    const shouldReplaceMessage = Boolean(replacementMessageId && replacementMessage);
	    ...
	    const mediaMessageHideAvatar = shouldReplaceMessage
	      ? Boolean(replacementMessage?.meta?.hideAvatar)
	      : (reuseSourceMessage ? false : sender.hideAvatar);
	    const mediaMessageCompactWithSource = shouldReplaceMessage
	      ? Boolean(replacementMessage?.meta?.compactWithSource)
	      : (reuseSourceMessage ? false : sender.hideAvatar);
	    const pendingMeta = {
	      ...(reuseSourceMessage ? (sourceMessage?.meta || {}) : {}),
	      ...(shouldReplaceMessage ? (replacementMessage?.meta || {}) : {}),
	      hideAvatar: mediaMessageHideAvatar,
    ...
    const insertAfterMessageId = sourceMessageId
      ...
      : '';
	    const savedPending = reuseSourceMessage
	      ? (chatStore.updateMessage(sourceMessageId, pendingMessage, sessionId) || { ...sourceMessage, ...pendingMessage, id: sourceMessageId })
	      : (shouldReplaceMessage
	      ? (chatStore.updateMessage(replacementMessageId, pendingMessage, sessionId) || { ...replacementMessage, ...pendingMessage, id: replacementMessageId })
  ...
  const retryChatGeneratedMediaFailure = async ({
    ...
    const message = chatStore.findMessage(mid, sid);
    ...
    return runChatImageGeneration({
      ...
      generationParamOverrides: generated.generationParams && typeof generated.generationParams === 'object'
        ...
        : {},
      autoGenerated: generated.autoGenerated === true,
  ...
  const patchWritingAutoImagePromptToken = (message, sessionId, pendingToken = '', replacement = '', extraMeta = {}) => {
    ...
    const { stored, display } = applyCreativeOutputRegexPairSafe(displaySource, {
      appBridge: window.appBridge,
      depth: 0,
      normalizeText: normalizeCreativeLineBreaksForDisplay,
    });
  ...
  const bindWritingAutoImagePromptRetryActions = () => {
    ...
    document.addEventListener('click', (event) => {
      const generatedRetry = event?.target?.closest?.('.generated-media-error-retry[data-action="retry-generated-media"]');
      if (generatedRetry) {
        event.preventDefault?.();
        event.stopPropagation?.();
        const wrapper = generatedRetry.closest?.('[data-msg-id]');
        const message = wrapper?.__chatappMessage && typeof wrapper.__chatappMessage === 'object'
          ? wrapper.__chatappMessage
          : null;
        retryChatGeneratedMediaFailure({
          sessionId: String(message?.sessionId || chatStore.getCurrent?.() || '').trim(),
          messageId: String(wrapper?.dataset?.msgId || message?.id || '').trim(),
        }).catch(err => {
          logger.warn('retry generated media failed', err);
	  ...
	  const deleteGeneratedImageAssetFromCurrentChat = async (asset = {}, options = {}) => {
	    ...
	    targets.forEach((message) => {
	      ...
	      if (inlineImages.length && String(asset?.scope?.surface || '') === 'writing') {
	        ...
	        const { stored, display } = applyCreativeOutputRegexPairSafe(nextContent || ' ', {
	          appBridge: window.appBridge,
	          depth: 0,
	          normalizeText: normalizeCreativeLineBreaksForDisplay,
	        });
  ...
  const buildRpGreetingMessage = (greeting, sessionId) => {
    ...
    ({ stored, display } = applyCreativeOutputRegexPairSafe(macroContent, {
      appBridge: window.appBridge,
      depth: 0,
      ...
      if (hadUpdateVariableTag) {
        logger.debug(
	      ...
	      if (isCreativeAssistant) {
	        ...
	        const { stored, display } = applyCreativeOutputRegexPairSafe(finalSource, {
	          appBridge: window.appBridge,
	          depth: 0,
  ...
  const clampWallpaperOpacity = value => {
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) return 1;
    return Math.min(1, Math.max(0, numeric));
  };

  const syncWallpaperOpacityUi = () => {
    const opacity = clampWallpaperOpacity(wallpaperState.opacity);
    if (wallpaperOpacityInput) wallpaperOpacityInput.value = String(opacity);
    if (wallpaperOpacityValue) wallpaperOpacityValue.textContent = `${Math.round(opacity * 100)}%`;
    if (wallpaperPreviewImage) {
      wallpaperPreviewImage.style.opacity = wallpaperState.previewUrl ? String(opacity) : '0';
    }
  };
  ...
  const applyWallpaperTransform = (imgEl, containerEl, meta) => {
    ...
    imgEl.style.opacity = String(clampWallpaperOpacity(meta.opacity ?? 1));
  };
  ...
  const normalizeChatSettings = raw => {
    ...
    if (raw?.wallpaper && typeof raw.wallpaper === 'object') {
      base.wallpaper = {
        ...raw.wallpaper,
        opacity: clampWallpaperOpacity(raw.wallpaper.opacity ?? 1),
      };
      return base;
    }
  ...
  const applyWallpaperPreviewTransform = () => {
    ...
    wallpaperPreviewImage.style.opacity = String(clampWallpaperOpacity(wallpaperState.opacity));
  };
  ...
  const resetWallpaperState = (next = {}) => {
    ...
    wallpaperState.opacity = clampWallpaperOpacity(next.opacity ?? 1);
  ...
  const loadWallpaperEditor = (sessionId, settings) => {
    ...
    const init = {
      ...
      opacity: clampWallpaperOpacity(current?.opacity ?? 1),
      dirtyTransform: false,
      current,
    };
    init.initial = { ...init };
        ...
        if (resp?.path) {
          ...
          return {
            wallpaper: {
              ...
              opacity: clampWallpaperOpacity(wallpaperState.opacity),
              updatedAt: Date.now(),
            },
          };
        }
  ...
  wallpaperOpacityInput?.addEventListener('input', e => {
    wallpaperState.opacity = clampWallpaperOpacity(e.target?.value ?? 1);
    markWallpaperDirty();
    syncWallpaperOpacityUi();
  });
