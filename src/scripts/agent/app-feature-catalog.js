...
export const APP_FEATURE_DEFINITIONS = Object.freeze([
  ...
  },
  {
    id: 'session.list',
    title: '查看会话列表',
    aliases: ['会话列表', '聊天室列表', '列出会话', '列出聊天室', '有哪些聊天室', '有哪些会话', '查看所有聊天室'],
  ...
  },
  {
    ...
    argsHint: 'resource 指定 chat/worldbook/regex/variables/memory/preset/config/session/persona/user；worldbook 默认返回条目索引、全局设置与 AI 生成模板，需正文时传 includeContent:true、entryId、entryTitle 或 query；persona/user 默认只返回 id/name/active，需描述、头像或完整档案时分别传 include:["description"]、include:["avatar"]、include:["details"]；sessionId/sessionName/target/chatName/id/name/limit/include 可选；读取聊天时可用聊天室名称定位',
    panel: '',
    riskLevel: 'low',
    writes: false,
    confirmation: 'allow_once',
