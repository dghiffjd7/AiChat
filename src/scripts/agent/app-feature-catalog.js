...
export const APP_FEATURE_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    argsHint: 'persona.set_avatar: target/personaId/name 可指定角色卡；attachmentId 可指定本次附图，省略时使用第一张图；不要把 base64 写入 args；目标已有头像时必须由用户点击确认后才会覆盖。用户要求现生成头像时：先调用 media.generate_image（prompt 必填、negativePrompt 可选）取得 attachmentId，再调用 persona.set_avatar；不要改用 chat.generate_image，否则只会把图片发进聊天室而不会设置头像',
    panel: 'persona',
    riskLevel: 'medium',
    writes: true,
    confirmation: 'allow_once',
  ...
  },
  {
    ...
    argsHint: 'session.set_wallpaper: target/sessionId/sessionName/chatName/name 可指定聊天室；attachmentId 可指定本次附图，省略时使用第一张图；opacity 可选；不要把 base64 写入 args；目标已有壁纸时必须由用户点击确认后才会覆盖。用户没发图但要求联网找图时：先 web.search_images 搜索——动漫角色传 tags 用 booru 标签格式（如 rem_(re:zero)、tifa_lockhart）并设 style:anime；壁纸设 purpose:wallpaper；真人/写实设 style:photo。然后 media.fetch_image 下载所选 imageUrl 得到 attachmentId，再设置。用户要求现生成壁纸时：先调用 media.generate_image（prompt 必填、negativePrompt 可选）取得 attachmentId，再调用 session.set_wallpaper；不要改用 chat.generate_image，否则只会把图片发进聊天室而不会设置壁纸',
    panel: 'session-config',
    riskLevel: 'medium',
    writes: true,
    confirmation: 'allow_once',
  ...
  },
  {
    ...
    argsHint: 'worldbook.delete_entries: name/worldbookId/id/sessionId 指定世界书；entries/deletes 可按 entryId/entryTitle/title/query 显式删除；清理同名重复条目改用 dedupeByTitle:true、duplicateTitles/titles 指定标题、keep:first/last 指定保留哪一条。显式 entries/deletes 与 dedupeByTitle 两种模式禁止混用，混用会 fail closed；删除前必须用户确认，删除后用 worldbook.read 验证',
    panel: 'worldbook',
    riskLevel: 'high',
    writes: true,
    confirmation: 'allow_once',
