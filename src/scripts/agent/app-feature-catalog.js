...
export const APP_FEATURE_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    argsHint: 'name/worldbookId/id 指定世界书；省略时读取当前会话世界书；maxEntries/maxContentLength 控制返回量',
    panel: 'worldbook',
    riskLevel: 'low',
    writes: false,
    confirmation: 'allow_once',
  ...
  },
  {
    ...
    argsHint: 'resource 指定 chat/worldbook/regex/variables/memory/preset/config/session/persona/user；sessionId/sessionName/target/chatName/id/limit/include 可选；读取聊天时可用聊天室名称定位',
    panel: '',
    riskLevel: 'low',
    writes: false,
    confirmation: 'allow_once',
  ...
  },
  {
    ...
    summary: '在用户询问当前、外部或公开网络信息时搜索网页，并可读取指定网页正文；不要用于读取 APP 私有数据。',
    uiPath: [],
    tools: ['web.search', 'web.fetch_url'],
    argsHint: 'web.search: query 必填，limit 可选；web.fetch_url: url 必填，maxTextLength 可选',
    panel: '',
    riskLevel: 'low',
    writes: false,
    confirmation: 'allow_once',
