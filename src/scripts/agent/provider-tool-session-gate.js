...
export const writeProviderToolSessionGate = ({
  ...
  const next = normalizeProviderToolSessionGate({
    ...
    modelContextPolicy,
    networkAllowed: enabled === true && networkAllowed === true,
    realRunnerAllowed: enabled === true && realRunnerAllowed === true,
    writesChat: false,
    source,
    reason,
  ...
  settings[PROVIDER_TOOL_SESSION_GATE_SETTINGS_KEY] = {
    ...
    networkAllowed: next.networkAllowed,
    realRunnerAllowed: next.realRunnerAllowed,
    writesChat: false,
    source: next.source,
    reason: next.reason,
