(async () => {
  ...
  const ids = [...code.matchAll(/id="([^"]+)"/g)]
    .filter(x => !inScript(x.index))
    .map(x => ({ id: x[1], at: x.index }));
  const missing = ids.filter(x => !doc.getElementById(x.id));
  const present = ids.filter(x => doc.getElementById(x.id));
  const lastPresentAt = Math.max(...present.map(x => x.at));
  const firstMissingAt = Math.min(...missing.map(x => x.at));
  return {
    totalStaticIds: ids.length, present: present.length, missing: missing.length,
    lastPresentAt, firstMissingAt,
    docLen: code.length,
    aroundBoundary: code.slice(Math.max(0, firstMissingAt - 300), firstMissingAt + 80).replace(/\n\s*/g, ' ').slice(0, 350),
  };
})()
