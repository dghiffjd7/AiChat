...

{
  ...
  assert.equal(tauriConfig.version, '0.7.2');
  ...
  assert.match(cargoToml, /^version = "0\.7\.2"$/m);
  assert.match(cargoToml, /^name = "omnitavern_lib"$/m);
  assert.match(rustMain, /omnitavern_lib::run\(\)/);
  assert.match(cargoLock, /name = "omnitavern"\s+version = "0\.7\.2"/);
...

{
  ...
  assert.match(webSearchTools, /OmniTavern-Maid-Assistant\/0\.7\.2/);
