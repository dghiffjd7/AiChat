...
assert.match(source, /\.maid-onboarding-hint\s*\{[\s\S]*?grid-template-columns:\s*50px minmax\(0, 1fr\) 40px;/);
assert.match(source, /\.maid-onboarding-hint-avatar\s*\{[\s\S]*?width:\s*50px;[\s\S]*?height:\s*50px;/);
assert.match(source, /\.maid-onboarding-welcome-avatar-wrap\s*\{[\s\S]*?width:\s*52px;[\s\S]*?height:\s*52px;/);
assert.match(source, /\.maid-onboarding-welcome-avatar\s*\{[\s\S]*?width:\s*52px;[\s\S]*?height:\s*52px;/);
assert.match(source, /\.maid-onboarding-toast-icon\s*\{\s*width:\s*46px;\s*height:\s*46px;/);
